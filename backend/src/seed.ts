...
async function main() {
  ...
  if (existingQuestionnaire) {
    ...
    console.log(
      "Testovací dotazník už existuje:",
      questionnaire.id
    );
  } else {
    questionnaire =
      await prisma.questionnaire.create({
        data: {
          name: "Krátky dotazník nálady",
          description:
            "Jednoduchý testovací dotazník na overenie administrácie.",
          status: "ACTIVE",
          language: "sk",
          estimatedMinutes: 2,
        },
  ...
  const questions = [
    ...
    {
      text:
        "Ako často ste sa v posledných dňoch cítili nervózny/nervózna?",
      order: 2,
      options: [
        "Nikdy",
        "Zriedka",
    ...
    {
      text:
        "Ako hodnotíte svoju celkovú spokojnosť?",
      order: 3,
      options: [
  ...
  for (const questionData of questions) {
    await prisma.question.create({
      data: {
        ...
        options: {
          create: questionData.options.map(
            (text, index) => ({
              text,
              value: index + 1,
              order: index + 1,
  ...
  console.log(
    "Vytvorená verzia dotazníka:",
    version.id
  );

  console.log(
    "Vytvorené 3 testovacie otázky."
  );
}
